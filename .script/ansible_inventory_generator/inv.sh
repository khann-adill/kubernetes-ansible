...
then
        ...
        echo $1 ansible_python_interpreter=/usr/bin/python3 >> /etc/ansible/custom_inv.ini
        echo "[worker]" >> /etc/ansible/custom_inv.ini
        for node_host in $@
        do
                echo $node_host ansible_python_interpreter=/usr/bin/python3 >> /etc/ansible/custom_inv.ini
        done
